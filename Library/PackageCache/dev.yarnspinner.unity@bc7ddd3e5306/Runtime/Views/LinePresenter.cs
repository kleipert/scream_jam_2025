...
namespace Yarn.Unity
{
    ...
    public sealed class LinePresenter : DialoguePresenterBase
    {
        ...
        const double LeadInSeconds = 0.5;   // 50 ms, ggf. 0.03–0.08 feinjustieren
        const double EndGuard = 0.03;        // 30 ms „Sicherheitsnetz“ fürs Stoppen
        ...
        public override async YarnTask RunLineAsync(LocalizedLine line, LineCancellationToken token)
        {
            ...
            if (useTypewriterEffect)
            {
                ...
                // --- AUDIO START: per DSP exakt terminieren ---
                System.Action stopAudioNow = () => { }; // Fallback, falls kein Clip gesetzt

                if (typewriterAudio != null)
                {
                    var tStart = AudioSettings.dspTime + LeadInSeconds;
                    
                    typewriterAudio.loop = true;     // langer Loop-Clip
                    typewriterAudio.PlayScheduled(tStart);

                    // präzises Beenden vorbereiten
                    stopAudioNow = () =>
                    {
                        // Sofortiges Stoppen so präzise wie möglich (DSP-zeitnah)
                        var tNow = AudioSettings.dspTime + EndGuard;
                        // Bei Loops ist SetScheduledEndTime präziser als Stop()
                        typewriterAudio.SetScheduledEndTime(tNow);
                    };

                    // Grafik/Typewriter erst NACH dem geplanten Audiostart starten
                    // -> so laufen beide synchron los
                    await YarnTask.Delay((int)(LeadInSeconds * 1000));
                }
